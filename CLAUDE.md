...
- **Data model authority**: `MCD.dbml` at the repo root is the canonical entity-relationship spec; `docs/plan-initial.md` is the original product/architecture plan (sync scenarios A/B/C referenced in code comments live there) — a **historical document**, accurate on the data model and sync design, outdated on auth and roadmap. **Keep `MCD.dbml` up to date**: whenever you change the data model (new/renamed tables, columns, enums, or relationships in `db_models.py` / migrations), update `MCD.dbml` in the same change so it stays an accurate, reviewable mirror of the schema — it's the reference used during code review.
